...
class AppointmentStatusForm(forms.Form):
    choose_status = forms.CharField(required=False, label="choose_status")

    @transaction.atomic
    def save(self, offer_appointment):
        offer_appointment.status = self.cleaned_data.get("choose_status")
        offer_appointment.save()
